...
const SahayakMode: React.FC<{ language: Language; userKey: string | null; onListeningStateChange?: (isListening: boolean) => void }> = ({ language, userKey, onListeningStateChange }) => {
  ...
  const speakResult = async (text: string, forcePlay = false) => {
    ...
    try {
      ...
      // Final fallback: Try English (Indian) then English (US)
      if (!matchingVoice) matchingVoice = tryFind(v => v.lang === 'en-IN');
      if (!matchingVoice) matchingVoice = tryFind(v => v.lang === 'en-US' || v.lang === 'en-GB');

      if (matchingVoice) {
        utterance.voice = matchingVoice;
        setVoiceDebug(`Voice: ${matchingVoice.name} (${matchingVoice.lang})`);
      } else {
        setVoiceDebug(`No suitable voice found. Using system default.`);
        // No alert, just let it try default
      }
  ...
  const SUGGESTIONS = [
    { key: 'fir', label: t.topics.fir, icon: ICONS.POLICE, hint: "FIR Help" },
    { key: 'property', label: t.topics.property, icon: ICONS.HOME, hint: "Land Issue" },
    { key: 'fraud', label: t.topics.fraud, icon: ICONS.MONEY, hint: "Money Lost" },
    { key: 'bail', label: t.topics.bail, icon: ICONS.UNLOCK, hint: "Bail Info" },
    { key: 'violence', label: t.topics.violence, icon: ICONS.SHIELD, hint: "Safety" }
  ];
            ...
            <div className="max-w-5xl mx-auto w-full px-6 py-8 sm:py-16 space-y-12 animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                ...
                <button
                  ...
                  className="group relative flex flex-col items-center justify-center p-10 sm:p-14 bg-[var(--legal-black)] text-[var(--legal-white)] rounded-[2.5rem] transition-all hover:scale-[1.02] active:scale-[0.98] shadow-2xl overflow-hidden"
                >
                  <div className="absolute top-0 right-0 w-40 h-40 bg-[var(--legal-white)]/5 rounded-full -mr-16 -mt-16 blur-3xl transition-all duration-500"></div>
                  <div className="mb-6 scale-110 sm:scale-125 text-[var(--legal-gold)] group-hover:rotate-12 transition-transform duration-500 drop-shadow-lg">{ICONS.MIC}</div>
              ...
              <div className="space-y-6 pt-4 max-w-4xl mx-auto">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  {SUGGESTIONS.map((s, idx) => (
                    <button
                      key={idx}
                      onClick={() => { setContextData(''); setTranscript(s.label); setActiveTopic(s.key); setState('THINKING'); }}
              ...
              <div className="bg-[var(--legal-black)] text-[var(--legal-white)] rounded-[2.5rem] p-6 sm:p-10 shadow-2xl relative overflow-hidden">
                ...
                <div className="flex flex-col gap-6 relative z-10">
                  ...
                  <h1 className="text-2xl sm:text-4xl md:text-5xl font-serif italic text-[var(--legal-white)] leading-tight font-native">
                    "{fileName ? fileName : (activeTopic ? t.topics[activeTopic as keyof typeof t.topics] : (transcript || textInput))}"
                  </h1>
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-[var(--legal-accent)]">
                ...
                {!showLawyers && !showFIR ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="flex flex-col gap-4 p-6 bg-[var(--legal-black)] rounded-[2rem] border border-[var(--legal-gold)]/20 hover:border-[var(--legal-gold)] transition-all group relative overflow-hidden shadow-xl">
                      <div className="absolute top-0 right-0 p-4 opacity-10 text-[var(--legal-gold)] group-hover:opacity-20 transition-opacity">
                        <div className="scale-150">{ICONS.REFRESH}</div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="p-3 bg-[var(--legal-gold)]/20 text-[var(--legal-gold)] rounded-xl group-hover:scale-110 transition-transform">
                          {ICONS.REFRESH}
                        </div>
                        <div>
                          <h4 className="text-sm font-bold text-[var(--legal-gold)]">{t.btn_new}</h4>
                          <p className="text-[10px] text-slate-400 font-medium">{t.placeholder_type || "Start a Fresh Query"}</p>
                        </div>
                      </div>
                      <button
                        onClick={() => {
